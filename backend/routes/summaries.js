...
function enrichSummary(s) {
  ...
  s.author = author?.name || ''; s.author_username = author?.username || ''; s.authorData = author || null;
  return s;
}
...
router.get('/feed', optionalAuth, (req, res) => {
  ...
  if (!u) {
    ...
    return res.json(rows.map(enrichSummary));
  }
  ...
  res.json([...primary, ...promoted.filter(s => !seen.has(s.id))].map(enrichSummary));
});
...
router.get('/', optionalAuth, (req, res) => {
  ...
  res.json(db.prepare(`SELECT s.* FROM summaries s WHERE ${where.join(' AND ')} ORDER BY ${order} LIMIT 200`).all(...params).map(enrichSummary));
});
